...
\begin{abstract}
This paper considers the decentralized motion planning under tight coupled task specifications of multi-agent systems. Multi-agent systems consist of heterogeneous agents. The assigned tasks are specified as Linear Temporal Logic (LTL) formulae. A decentralized motion planning scheme is proposed to alleviate the massive computational complexity of the centralized multi-agent motion planning. The couple-edges are defined to eliminate the coupling among tight tasks. The online collaboration scheme based on the real-time communication is employed to ensure the satisfaction of task specifications without the central collaboration. In some scenarios such as the task swapping and inheriting, the union agent model is proposed to plan the motion paths and reduce the computational complexity. The overall scheme is demonstrated by a simulation in a warehouse scenario and 14 agents are assigned tight coupled task specifications.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
multi-agent systems, linear temporal logics (LTLs), decentralized motion planning.
\end{IEEEkeywords}
...
LTL has been employed in multi-agent systems to plan the motion paths under complex task specifications. LTL is able to model the complex logic. The complete solution space can be spanned through the specifications. Meanwhile, the specifications can be relaxed when the deadlock occurs \cite{guo2013reconfiguration}. In \cite{Chen2013A} and \cite{chen2012formal}, the essential LTL is employed by a multi-agent system to collaborate in the Robotic Urban-Like Environment(RULE), which establishes the scheme of the application of LTL in the static workspace. With respect to the dynamic workspace, agents need to revise their motion paths. A motion path revision method based on knowledge transferring and real time DijksTA algorithm is proposed in \cite{guo2013revising} and \cite{guo2015multi}. If the LTL specifications were infeasible, \cite{guo2013reconfiguration} relaxes the infeasible specifications and synthesizes the motion paths that fulfill the infeasible specifications most. \par
A key challenge of applying LTL to the motion planning of multi-agent systems is the reduction of the massive computational complexity, which is caused by the production operation. In centralized motion planning method, each agent needs to synthesize its local product automaton, whose complexity exponential increases with the number of states. Then all the agents need to synthesize the final agent model through the production of all the local product automata, which causes the states number of final agent model to increase exponentially with the number of agents. So it costs a long duration to plan the motion paths in the scenarios, where the task is complex and the number of agents is large. The massive computational complexity restricts the application of LTL in practice.
...
In this section, the decentralized motion planning scheme is proposed to decentralize the synthesis of initial motion paths. The scheme contains two major parts: 1) the decoupling of tight coupled task specifications; 2) the real-time communication to guarantee the collaboration.
...
Given $\mathcal{C}^{p_i}_n$ and $\varphi^{p_i}$, there exists a finite path satisfying $\varphi^{p_i}$ if and only if $\mathcal{C}^{p_i}_n$ has a finite path from an initial state to an accepting state. Let $\varrho^{p_i}=q^{p_i,\ast}_0 q^{p_i,\ast}_1 ... q^{p_i,\ast}_s$ be a finite path, where $q^{p_i,\ast}_0\in Q^{p_i,\ast}_0$, $q^{p_i,\ast}_s\in Q^{p_i,\ast}_F$ and $q^{p_i,\ast}_z\in Q^{p_i,\ast}$ and $(q^{p_i,\ast}_z,q^{p_i,\ast}_{z+1})\in \delta^{p_i}$. The $cost$ of $\varrho^{p_i}$ is denoted by $cost(\varrho^{p_i},\mathcal{C}^{p_i}_n)=\sum^{s}_{k=0}W^{p_i,\ast}(q^{p_i,\ast}_k,q^{p_i,\ast}_{k+1})$. Through the application of the Algorithm 1 in \cite{guo2013motion},which utilizes DijksTA algorithm, the shortest accepting path can be found.
...
\textbf{\emph{Theorem 1:}} The proposed scheme solves Problem 1.

\textbf{\emph{proof:}} The theorem will be proof through two steps. First, we proof that through adding all the couple-edges found out by applying Algorithm 1 to $\mathcal{C}^{p_i}$, each agent can synthesize an initial accepting motion path.
...
Next, we proof that the initial motion paths induced by the centralized method are all contained in the paths generated by the decentralized scheme. Namely, the proposed decentralized path synthesis method has solutions as long as the original centralized method has solutions.

In Algorithm 1 all the couple-edges have been added into $\mathcal{C}^{p_i}$. Then towards $\mathcal{C}^{p_i}_n$, the valid task specification becomes $\varphi \setminus \varphi_c$. Synthesize an initial motion path $\varrho^{p_i}$ through the original centralized plan synthesis method, $\varrho^{p_i}$ fulfills $\varphi$ and $\varphi \setminus \varphi_c$ must be fulfilled simultaneously. So the solution space spanned by the original centralized method are contained in the solution space generated by the decentralized method. Consequently, the proposed decentralized path synthesis method has solutions as long as the original centralized method has solutions. So the proposed decentralized scheme solves Problem 1. \par
...
\textbf{\emph{Theorem 2:}} The $R^{\varpi}_+$ can always be synthesized through the union agent model $\widetilde{\mathcal{U}}_{ij}$.

\textbf{\emph{proof:}} Based on the assumption that part of the local FTS $\mathcal{F}^{p_i}$ and $\mathcal{F}^{p_j}$ are bordered and Theorem 1, there must be a minimum spanning tree in $\widetilde{\mathcal{U}}_{ij}$, so the $R^{\varpi}_+$ can always be synthesized and Problem 2 is solved.
...
This paper presents a decentralized motion planning scheme to alleviate the massive computational complexity. The scheme relies on the decoupling of coupled task specifications and real-time communication. Towards the scenarios the agents need to swap their tasks or some tasks need to be inherited, the union agent model is proposed to avoid the product operation. The proposed method can handle the decentralized collaboration of multi-agent system under tight coupled task specifications. And the computational complexity of the motion planning is reduced significantly compared to the centralized method. Future work is focused on the decentralized motion planning under hard and soft constraints, which restrict the motion of agents and the communication among agents during the collaboration.
