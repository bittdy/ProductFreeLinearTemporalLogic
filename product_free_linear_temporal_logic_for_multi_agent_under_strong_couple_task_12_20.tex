...
\begin{abstract}
The local motion planning under compact coupled task specifications of a multi-agent system that consists of heterogeneous groups of homogeneous agents is considered in this paper. To alleviate the massive computational complexity of centralized multi-agent planning, a decentralized solution is proposed, where couple-edge is proposed to eliminate the coupling among compact task and transform the collaboration issue into a local task planning problem. Meanwhile, we adapt the online collaboration scheme based on real-time exchange of request and reply messages to ensure the satisfaction of task specifications without central collaboration. This adapted online collaboration scheme facilitates decreasing the total execution cost in general collaboration scenario. Towards scenarios where partial agents are unstable, union agent model based on previous decentralized solution is proposed to dispatch agents to strive to complete task when part of agents failure. Proposed approach is demonstrated by a simulated scenario in a warehouse and agents are assigned compact coupled task specifications.
\end{abstract}
...
\textbf{\emph{Remark 3:}} Actually, the effect of above method is the same as the union of $\mathcal{F}^{p_j}$ and $\mathcal{C}^{p_i}_n$. We deposit the $\tilde{c}_{p_j}|_{\mathcal{B}^{p_j}}$ instead of abandoning them in order to use the same model to plan $p_j$ to go back to its own task after $p_i$ is repaired.
...
\caption{(a) and (b) show the motion paths of agents which start from different positions to inherit the good collection task.}
\label{third}
\end{figure}
