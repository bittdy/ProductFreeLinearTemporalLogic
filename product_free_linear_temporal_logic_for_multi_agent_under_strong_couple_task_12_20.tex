...
\begin{abstract}
This paper considers the decentralized motion planning under compact coupled task specifications of the multi-agent systems consist of heterogeneous groups of homogeneous agents. The assigned tasks are specified as Linear Temporal Logic (LTL) formulas. A decentralized motion planning method is proposed to alleviate the massive computational complexity of centralized multi-agent motion planning, where couple-edge is proposed to eliminate the coupling among compact task and transform the collaboration issue into a local task planning problem. Meanwhile, the online collaboration scheme based on real-time exchange of request and reply messages is adapted to ensure the satisfaction of task specifications without central collaboration. This adapted online collaboration scheme facilitates decreasing the total execution cost in general collaboration scenario. Towards scenarios where partial agents are unstable, union agent model based on product-free decentralized solution is proposed to dispatch agents to strive to complete task when part of agents fail. Proposed approach is demonstrated by a simulated scenario in a warehouse and agents are assigned compact coupled task specifications.
\end{abstract}
