...
from simu_buchi import check_label_for_buchi_edge
...
class ProdAut_Run(object):
	...
	def prod_run_to_prod_edges(self, product):
		self.pre_prod_edges = [i for i in zip(self.prefix[0:-2], self.prefix[1:-1])]
		self.suf_prod_edges = [i for i in zip(self.suffix[0:-2], self.suffix[1:-1])]
	...
	def plan_output(self, product):
		...
		if len(self.line) == 2:
			...
			self.pre_ts_edges = [i for i in zip(self.line[0:-1], self.line[1:])]
		if len(self.loop) == 2:
			self.suf_ts_edges = [(self.loop[0], self.loop[1])]
		else:
			self.suf_ts_edges = [i for i in zip(self.loop[0:-1], self.loop[1:])]
