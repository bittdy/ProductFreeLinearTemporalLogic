import sys
sys.path.append('/home/bittdy/own_code/simu_1_614/boolean_formulas')
...
class NotSymbolExpression(Expression):
    ...
    def check(self, label):
        truth = self.symbol not in label
        symbol_list = []
        if self.symbol in label:
            symbol_list.append(self.symbol)
        return truth,symbol_list
...
class ORExpression(BinExpression):
    ...
    def check(self, label):
        truth_left,list_left = self.left.check(label)
        truth_right,list_right = self.right.check(label)
        return truth_left or truth_right,list_left + list_right
...
class ANDExpression(BinExpression):
    ...
    def check(self, label):
        truth_left,list_left = self.left.check(label)
        truth_right,list_right = self.right.check(label)
        return truth_left and truth_right,list_left + list_right
