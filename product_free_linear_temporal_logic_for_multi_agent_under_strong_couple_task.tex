...
where the meaning of notations in [2] are the same as the notations in [3]. The agent $p_j$ that minimizes [2] in group $g_s$, $p_i,p_j \in g_s$, is elected to succeed $p_i$.\par
...
\emph{Problem 2:} With a little symbol abuse, we denote the states in $\mathcal{C}^{p_i}_n$ by $\tilde{c}_{p_i}$ and assume the motion plan of $p_j$ after revision is $R^{p_j}_+$ and the original one is $R^{p_j}_-$. Given the local complete agent model $\mathcal{C}^{p_i}_n$ and $\mathcal{C}^{p_j}_n$, decide the grade $\varpi$ of the agent inheriting the task of $p_i$ and construct $R^{\varpi}_+$ to minimize the total cost. \par
$\mathcal{C}^{p_i}_n$ and $\mathcal{C}^{p_j}_n$ can be combined to obtain $union\ agent\ model$. Assume that part of the local FTS $\mathcal{F}^{p_i}$ and $\mathcal{F}^{p_j}$ of $p_i$ and $p_j$ are bordered and then the construction procedure of union agent model $\widetilde{\mathcal{U}}_{ij}$ is as following:\par
...
zhe li yao shuo ming, yao fang qi yuan lai de ren wu,dan shi yi xie ying xing renwu yao bao liu. ran hou qi ta de ren wu yao cheng jie ji cheng de ren wu. huo zhe zai qian mian zai jia ge shuo ming. dui yu zheng ge huan jing nei de ying xing yue shu, suo you ji qi ren de ren wu mo xing zhong dou you.
...
\begin{algorithm}
  ...
  \ForEach{$\tilde{c}_{p_j}\in \mathcal{C}^{p_j}_n$}
  {
    ...
    {
        \If{$\tilde{c}_{p_i}|_{\mathcal{F}^{p_i}}\ is\ borded\ to\ \tilde{c}_{p_i}|_{\mathcal{F}^{p_j}}$}
        {
            ...
            {
                $\tilde{c}_{p_j}\in \delta^{p_i}(\tilde{c}_{p_i})$

                $W_{\mathcal{U}_{ij}}(\tilde{c}_{p_i},\tilde{c}_{p_j})=dist(\tilde{c}_{p_i}|_{\mathcal{F}^{p_i}},\tilde{c}_{p_j}|_{\mathcal{F}^{p_j}})$

                $\tau(\tilde{c}_{p_i},\tilde{c}_{p_j}) = None$
            }
            \Else
            {
                \If{$L^{p_i}(\tilde{c}_{p_j}|_{\mathcal{F}^{p_i}})\in sym(\varphi_c^{p_j})$}
                {
                    ...
                    $\tau(\tilde{c}_{p_i},\tilde{c}_{p_j})=L^{p_i}(\tilde{c}_{p_i}|_{\mathcal{F}^{p_i}})$
                }
            }
        }
    }
  }
  $R^{\varpi,p}_+=bidirection_dijkstra(\Omega_{p_j},\tilde{\eta}_{p_i})$
...
\subsection{Prevention Of Long-time Demission}
In this section we increase the amount of agents in $g_1$ to yin zheng suan fa 2 de he shi xing
