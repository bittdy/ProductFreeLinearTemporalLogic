...
\begin{algorithm}
  ...
  \KwOut{complete agent model $\mathcal{C}^{p_i}$}
...
\begin{algorithm}
  ...
  \KwIn{$\textbf{Request}_{p_i}^{\ast}$, $\textbf{Reply}_{p_i}^{\ast}$, $\textbf{Confirm}_{p_i}^{\ast}$, $\partial$, $\mathcal{R}^{p_i}_c$, $\mathcal{C}^{p_i}$ }
  \KwOut{$state^{p_i}$, $R^{p_j}_+$}
